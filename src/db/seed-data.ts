...
export async function seedDatabase() {
    ...
    await addTasks(level1.id, [
        ...
        },
        {
            ...
            description: "## 🏆 Ролі елементів\n\n**Теорія**: `getByRole` — це найбільш надійний селектор, який імітує те, як користувачі сприймають сторінку. Краще шукати кнопку за її роллю `button` і назвою, ніж за CSS класом.\n\n**Приклад**:\n```typescript\nimport { test, expect } from '@playwright/test';\n\ntest('click login', async ({ page }) => {\n  await page.getByRole('button', { name: 'Log in' }).click();\n});\n```\n\n### Завдання\nЗнайдіть кнопку з текстом 'Sign up' (або 'Реєстрація') та натисніть на неї.",
