...
export default function DashboardClient({ tracks, tasks, statusMap, isAdmin, userName, userImage, firstName, hasContacts, projects, role, userId, botUsername }: Props) {
    ...
    return (
        <div className="flex flex-col min-h-screen bg-background overflow-x-hidden font-sans relative">
            ...
            <div className="flex px-4 pt-6 pb-4 gap-4 relative z-10 w-full max-w-[1600px] mx-auto">
                <div className="flex-shrink-0 w-80 rounded-[2rem] overflow-hidden glass-card-premium border border-white/5 shadow-2xl sticky top-4 self-start h-[calc(100vh-5rem)] -mt-6 lg:-mt-10">
                    <Sidebar
                        tracks={tracks}
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-3 mb-1">
                                                            <div className="flex items-center gap-2">
                                                                <span className="px-2 py-0.5 rounded-md bg-white/5 border border-white/10 text-[10px] font-black text-slate-500 tracking-widest uppercase">{selectedTrack.title.split(' — ')[0]} • ЗАВДАННЯ #{task.numbering || task.id}</span>
                                                                {/* Skill Tags */}
